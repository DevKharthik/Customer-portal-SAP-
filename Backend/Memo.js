...
const SAP_URL = 'http://AZKTLDS5CP.kcloud.com:8000/sap/bc/srt/scs/sap/zpc_memo_sd?sap-client=100';
const SAP_AUTH = {
  username: 'K901564',
  password: '06-Aug-030'
};
  ...
  try {
    ...
    xml2js.parseString(response, { explicitArray: false }, (err, result) => {
      ...
      try {
        ...
        const memoRes = Object.values(body)[0]; // <ZCP_INQUIRY577_FM.Response>

        const items = memoRes?.ET_MEMO?.item;
        ...
        const mappedMemo = list.map(memo => ({
          VBELN: memo.VBELN,
          FKART: memo.FKART,
          FKDAT: memo.FKDAT,
          NETWR: memo.NETWR,
