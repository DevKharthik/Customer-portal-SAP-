...
const SAP_URL = 'http://AZKTLDS5CP.kcloud.com:8000/sap/bc/srt/scs/sap/zpc_overall_sales_sd?sap-client=100';
const SAP_AUTH = {
  username: 'K901564',
  password: '06-Aug-030'
};
  ...
  try {
    ...
    xml2js.parseString(response, { explicitArray: false }, (err, result) => {
      ...
      try {
        ...
        const overRes = Object.values(body)[0]; // <ZCP_INQUIRY577_FM.Response>

        const items = overRes?.ET_SALES_SUMMARY?.item;
        ...
        const mappedSummary = list.map(summary => ({
